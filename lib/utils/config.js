 ...
 * @param {{app:function, client:function, server:function} config
 * @returns {{app:string, server:string, client:string}}
 */
module.exports.appConfig = function appConfig(filePath, config) {
    if (config && config.app && config.client && config.server) {
        return {
            app: config.app(filePath),
            server: config.server(filePath),
            client: config.client(filePath),
        };
    }
