...
class Renderer {
     ...
     * @property {String} expressVueFolder
     * @prop {MFS} MFS
    ...
    constructor(options = {}) {
        ...
        this.expressVueFolder = (options.expressVueFolder) ? 
            options.expressVueFolder :  path.join(this.rootPath, ".expressvue");
        this.distFolder = options.distFolder ? options.distFolder : null;
    }
    /**
    ...
    async _makeVueClass(filePath, context) {
        ...
        try {
            ...
            if(this.distFolder){
                context.script = new URL(path.relative(path.resolve('/'), bundlePath.memory.filename.client), this.distFolder).toString();
            } else {
                context.script = path.join('/', path.relative(path.resolve('/'), bundlePath.memory.filename.client)); 
            }
            ...
            if(this.distFolder){
                context.script = new URL(path.relative(path.resolve('/'), bundle.clientBundlePath), this.distFolder).toString();
            } else {
                context.script = path.join('/', path.relative(path.resolve('/'), bundle.clientBundlePath));
            }
            const layout = buildLayoutWebpack(context);
